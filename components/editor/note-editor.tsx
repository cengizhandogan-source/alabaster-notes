...
export function NoteEditor({ note }: { note: Note }) {
  const { toggle, isCollapsed, toggleCollapse, notes, tags, noteTags, githubLinks, githubRepos, githubConnected } = useSidebarContext()
  ...
  const noteGithubLinks = useMemo(() => githubLinks.filter((l) => l.note_id === note.id), [githubLinks, note.id])
  return (
    <Editor
      note={note}
      notes={noteRefs}
